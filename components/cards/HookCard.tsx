...
/** hook — one bold claim/question, huge type, vertically centered, signature device on. */
export function HookView({ card, entered, onAskAbout }: CardViewProps<HookCardT>) {
  ...
  return (
    <CardFrame card={card} entered={entered} onAskAbout={onAskAbout} align="center" gap={20}>
      ...
      {card.sub && (
        <Rise>
          <p className="font-body" style={{ margin: 0, fontSize: 19, lineHeight: 1.35, color: "var(--ink-2)", textWrap: "pretty", maxWidth: 340 }}>
            {card.sub}
          </p>
        </Rise>
      )}
